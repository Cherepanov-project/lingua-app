...
export class PostExerciseInsertTextApi extends OpenAPIRoute {
  schema = {
    tags: ['insert_text'],
  ...
  async handle(request: Request, env: Env) {
    ...
    try {
      ...
      const { id: providedId, level, sentence, missing_words } = RequestSchema.parse(body)

      let id = providedId

      if (id === undefined) {
        const result = await env.DB.prepare('SELECT MAX(id) AS maxId FROM insert_text').first<{
          maxId: number | null
        }>()
      ...
      const [inserted] = await db
        .insert(exerciseInsertTextTable)
        .values({ id, level, sentence, missing_words: JSON.stringify(missing_words) })
        .returning()
